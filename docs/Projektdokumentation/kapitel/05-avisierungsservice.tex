\section{Avisierungsservice -- Konzeption und Implementierung (TED)}
...
Kundenkommunikation und \texttt{Dispo\allowbreak{}Status\allowbreak{}Callback\allowbreak{}Service} für die
...
Bestätigungsanfragen und die darauf bezogenen Kundenreaktionen. Fachliche,
...
\begin{table}[htbp]
  \centering
  \caption{Zustandsdimensionen des Avisierungsmodells}
  \label{tab:zustandsdimensionen}
  \small
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}X@{}}
    \toprule
    Ebene & Objekt & Zustand / Merkmal & Bedeutung \\
    \midrule
    Fachlich & \texttt{Order}
      & \texttt{OPEN}, \texttt{SENT}, \texttt{CONFIRMED}, \texttt{REJECTED}, \texttt{NO\_RESPONSE}
      & Auftragsbezogener Stand der Avisierung \\
    Technisch & \texttt{Confirmation-}\newline\texttt{Request}
      & \texttt{PENDING}, \texttt{SENT}, \texttt{FAILED}
      & Technischer Versandstatus \\
    Gültigkeit & \texttt{Confirmation-}\newline\texttt{Request}
      & \texttt{active}
      & Freigabe für Kundenreaktionen; zusätzlich werden Frist, Aktualität und vorhandene Antwort geprüft \\
    Prozess & Camunda-Prozessinstanz
      & Workflowzustand
      & Technische Ablaufsteuerung, Wartezustände und Fortsetzungen \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{figures/persistenzmodell.png}
    \caption{Vereinfachtes Persistenzmodell des Avisierungsservice}
    \label{fig:persistenzmodell}
\end{figure}
...
\texttt{customer\_\allowbreak{}response.\allowbreak{}confirmation\_\allowbreak{}request\_\allowbreak{}id} begrenzt jede Anfrage
auf höchstens eine Kundenreaktion. Analog stellt die Eindeutigkeit von
\texttt{company\_\allowbreak{}email\_\allowbreak{}settings.\allowbreak{}company\_\allowbreak{}id} höchstens einen
...
Abfragen sowie den Zugriff auf die Bestätigungsanfragen eines Auftrags.
...
die Benachrichtigung des Kunden, die Verarbeitung der Kundenreaktion oder eines
...
Anwendungslogik übergeben. Ob dabei eine neue Bestätigungsanfrage und
...
Bestätigungsanfrage und vergleicht deren relevante Daten mit dem neu
...
Bestätigungsanfrage mit neuer Prozessinstanz erforderlich ist.
Tabelle~\ref{tab:avisierungsauftrag-entscheidung} fasst diese Entscheidung
zusammen.

\begin{table}[htbp]
  \centering
  \caption{Entscheidung bei der Übernahme eines Avisierungsauftrags}
  \label{tab:avisierungsauftrag-entscheidung}
  \small
  \setlength{\tabcolsep}{5pt}
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.43\textwidth}>{\raggedright\arraybackslash}X@{}}
    \toprule
    Ausgangslage & Verarbeitung \\
    \midrule
    Kein \texttt{Order} vorhanden
      & Auftrag mit \texttt{OPEN}, neue \texttt{PENDING}-Anfrage und Prozessinstanz anlegen \\
    Auftrag vorhanden, noch keine Anfrage
      & Auftrag aktualisieren und auf \texttt{OPEN} setzen; neue Anfrage und Prozessinstanz anlegen \\
    Letzte Anfrage \texttt{PENDING}, Daten unverändert
      & Vorhandene Verarbeitung unverändert weiterverwenden; kein neuer Prozess \\
    Letzte Anfrage \texttt{PENDING}, Daten geändert
      & Auftrag aktualisieren und auf \texttt{OPEN} setzen; bestehende Anfrage aktualisieren, Prozess beibehalten \\
    Letzte Anfrage \texttt{SENT} und aktiv, Daten unverändert
      & Bestehende Anfrage und aktuellen Auftragsstatus weiterverwenden \\
    Letzte Anfrage \texttt{SENT} und inaktiv, Auftrag \texttt{CONFIRMED} oder \texttt{REJECTED}, Daten unverändert
      & Vorhandenes Ergebnis weiterverwenden; keine neue Anfrage oder Prozessinstanz \\
    Alle übrigen Fälle, insbesondere \texttt{FAILED}, abgeschlossenes \texttt{NO\_RESPONSE} oder geänderte Daten bei \texttt{SENT}
      & Auftrag aktualisieren und auf \texttt{OPEN} setzen; aktive alte Anfrage deaktivieren und Ablösung signalisieren; neue \texttt{PENDING}-Anfrage und Prozessinstanz anlegen \\
    \bottomrule
  \end{tabularx}
  \par\smallskip
  \begin{minipage}{\textwidth}
    \footnotesize
    Maßgeblich ist die letzte Anfrage nach interner ID. „Daten unverändert“
    bedeutet: Sowohl Auftragsdaten (Tour, Kunde, Kontakte, Lieferadresse,
    Produkt, Menge und Preistext) als auch Anfragedaten (Lieferfenster,
    Kommunikationskanal und Antwortdauer) stimmen überein.
    Die Wiederverwendung einer aktiven \texttt{SENT}-Anfrage prüft hier
    \texttt{active}, nicht zusätzlich den Ablaufzeitpunkt.
  \end{minipage}
\end{table}
...
Ist eine neue Bestätigungsanfrage erforderlich, wird ein Token für den
...
Abbildung~\ref{fig:avisierungsservice-confirmation-request-process} dessen technische
...
\begin{figure}[htbp]
    ...
    \caption{Technische Workflow-Steuerung des Avisierungsprozesses in Camunda}
    \label{fig:avisierungsservice-confirmation-request-process}
\end{figure}

Das BPMN-Modell wird unter dem Schlüssel
\texttt{confirmation-request-process} deployt. Jede Prozessinstanz ist über die interne
\texttt{ConfirmationRequest.id} als Business Key genau einer
Bestätigungsanfrage zugeordnet. Derselbe Schlüssel wird zusammen mit dem
...
\begin{table}[htbp]
  \centering
  \caption{Technische Integration zwischen Camunda und Application Layer}
  \label{tab:workflow-integration}
  \small
  \setlength{\tabcolsep}{5pt}
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.27\textwidth}>{\raggedright\arraybackslash}X@{}}
    \toprule
    Bindeglied & Richtung / Geltungsbereich & Aufgabe \\
    \midrule
    Java Delegate & Camunda $\rightarrow$ Application
      & Use Case aufrufen; Ergebnisse als Prozessvariablen oder \texttt{BpmnError} in den Ablauf übertragen \\
    Workflow Service & Application $\rightarrow$ Camunda
      & Prozess starten; Kundenreaktion oder Ablösung per Message korrelieren \\
    Business Key & Beide Richtungen
      & Prozessinstanz über die interne \texttt{ConfirmationRequest.id} zuordnen; bei Korrelation zusätzlich Message-Namen verwenden \\
    Prozessvariablen & Innerhalb Camunda; Austausch über Adapter
      & Ablaufparameter wie Versandversuche, Retry-Abstand und Antwortfrist sowie Ergebnisdaten für die Fortsetzung bereitstellen \\
    \bottomrule
  \end{tabularx}
\end{table}
...
die jeweils neueste Bestätigungsanfrage eines Auftrags berücksichtigt.
...
Nach einem technisch erfolgreichen Versand wird die Bestätigungsanfrage in
...
\begin{table}[htbp]
  \centering
  \caption{Zustandsübergänge bei Kundenreaktion und Fristablauf}
  \label{tab:kundenreaktion-fristablauf}
  \small
  \setlength{\tabcolsep}{4pt}
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}X@{}}
    \toprule
    Ereignis & \texttt{Order} & \texttt{Confirmation-}\newline\texttt{Request} & \texttt{Customer-}\newline\texttt{Response} \\
    \midrule
    \texttt{CONFIRM} & \texttt{CONFIRMED}
      & \texttt{active = false}, Versandstatus \texttt{SENT}
      & Mit Typ \texttt{CONFIRM} gespeichert \\
    \texttt{REJECT} & \texttt{REJECTED}
      & \texttt{active = false}, Versandstatus \texttt{SENT}
      & Mit Typ \texttt{REJECT} gespeichert \\
    Fristablauf & \texttt{NO\_RESPONSE}
      & \texttt{active = false}, Versandstatus \texttt{SENT}
      & Keine Kundenreaktion angelegt \\
    \bottomrule
  \end{tabularx}
  \par\smallskip
  \begin{minipage}{\textwidth}
    \footnotesize
    Dargestellt sind zulässige Übergänge: Eine Kundenreaktion setzt die neueste,
    aktive, unbeantwortete und noch nicht abgelaufene Anfrage voraus.
    Beim Fristablauf muss die Anfrage aktiv und der Ablaufzeitpunkt erreicht sein.
  \end{minipage}
\end{table}
...
Versandfehler oder einem Fristablauf ohne Kundenreaktion eine erneute Avisierung
manuell über das Avisierungsdashboard angestoßen werden. Beide Wege können zur
Erzeugung einer neuen Bestätigungsanfrage führen, unterscheiden sich jedoch
...
\texttt{Confirmation\allowbreak{}Request\allowbreak{}Superseded} beendet. Die abgelöste Prozessinstanz
...
Wird aufgrund des bestehenden Zustands eine neue Bestätigungsanfrage
...
kein endgültiges Avisierungsergebnis vor. Eine Kundenentscheidung kann unmittelbar
...
Schritt. Ein technischer Versandfehler mit dem Versandstatus \texttt{FAILED}
sowie die Ablösung einer Bestätigungsanfrage durch eine neuere Avisierung
...
Bestätigungsanfrage an die Anwendungslogik zur externen Rückmeldung.

Vor dem HTTP-Aufruf wird geprüft, ob die zugrunde liegende
Bestätigungsanfrage noch aktuell ist. Hierzu wird die jüngste
...
Status. Wird beispielsweise nach einer Bestätigungsanfrage A aufgrund
...
Bestätigungsanfrage des Auftrags.
...
Auftrags-ID, der Bestätigungsstatus und, sofern vorhanden, ein vom Kunden
...
fehlgeschlagen behandelt wird. Vom Kommunikationsadapter gemeldete technische Versandfehler
werden grundsätzlich als wiederholbar behandelt. Eine feinere
...
Bestätigungsanfrage als \texttt{FAILED} markiert. Der Auftrag bleibt im
...
erfolgreich versendete Bestätigungsanfrage wird bei einer erneuten
...
Bestätigungsanfrage kann außerdem höchstens eine Kundenreaktion gespeichert
...
\subsection{Kanalunabhängige Kundenkommunikation (OKB)}
...
Bestätigungsanfrage kann über E-Mail, SMS oder WhatsApp versendet werden.
...
\begin{figure}[htbp]
  ...
  $\big\downarrow$
  \par\smallskip
  \commnode{\texttt{Channel\allowbreak{}Based\allowbreak{}Notification\allowbreak{}Service}}
  \par\smallskip
  ...
  $\big\downarrow$
  \par\smallskip
  \commnode{\texttt{Notification\allowbreak{}Channel\allowbreak{}Sender}}
  \par\smallskip
  \begin{tabular}{@{}ccc@{}}
...
die gemeinsame \texttt{Notification\allowbreak{}Delivery\allowbreak{}Exception} repräsentiert. Sie
...
Bestätigungsanfrage auch eine Eingangsbestätigung nach einer
...
umgesetzt. Die initiale Bestätigungsanfrage wird dagegen über alle drei
...
die initiale Bestätigungsanfrage sowie jeweils eines für die
...
Bestätigungsanfrage eine individuelle URL erzeugt. Nachrichtendarstellung
...
E-Mail mit Bestätigungsanfrage.
...
konfigurierte Absenderadresse. Die fachliche Erzeugung einer E-Mail mit Bestätigungsanfrage unter Verwendung
eines Thymeleaf-Templates sowie der Auftragsdaten und des Bestätigungslinks
ist davon nicht umfasst. Ein erfolgreicher Testversand zeigt
...
Zur Erweiterung der Kundenkommunikation um SMS und WhatsApp wurde
Twilio als externer Messaging-Provider eingesetzt. Ausschlaggebend
für diese Technologieentscheidung waren insbesondere die
unkomplizierte Anbindung über das verfügbare Java SDK sowie die
Möglichkeit, beide Kommunikationskanäle bereits während der
Entwicklungsphase mit einem begrenzten Trial-Account zu testen.
Dadurch konnten die vorgesehenen Kommunikationsabläufe ohne den
Aufbau einer eigenen Messaging-Infrastruktur zunächst unter
Entwicklungsbedingungen umgesetzt und überprüft werden.

Twilio stellt für SMS und WhatsApp eine gemeinsame
Messaging-Schnittstelle zur Verfügung. SMS-Nachrichten wurden im
Rahmen des Trial-Accounts über die Twilio Messages API von einer
konfigurierten Twilio-Rufnummer an eine verifizierte
Empfängerrufnummer gesendet. Für den Testbetrieb des
WhatsApp-Kanals wurde die von Twilio bereitgestellte WhatsApp
Sandbox verwendet \parencite{twilioMessagesResource,twilioWhatsAppSandbox}.

Die Nachrichteninhalte werden über ein zentral verwaltetes
Content Template definiert. Hierfür wurde im Twilio Content
Template Builder projektspezifisch das Template
\texttt{disposition\_test} angelegt. Das Template wird sowohl
für SMS als auch für WhatsApp verwendet und beim Versand über
seine Content-Template-SID referenziert. Dadurch konnten beide
Kommunikationskanäle weitgehend über eine gemeinsame technische
Implementierung angebunden und die verwendeten Nachrichteninhalte
zentral und kanalübergreifend gepflegt werden \parencite{twilioContentTemplates}.

Die technische Umsetzung baut auf der in
Abschnitt~\ref{sec:kommunikationsabstraktion} beschriebenen
Kanalauswahl auf. Abhängig vom gespeicherten \texttt{CommunicationChannel} delegiert die Anwendung den
Versand an den \texttt{SmsNotificationSender} oder den
\texttt{Whats\allowbreak{}App\allowbreak{}Notification\allowbreak{}Sender}. Beide Implementierungen
erzeugen aus dem Auftrag und der Bestätigungsanfrage einen
gemeinsamen \texttt{Confirmation\allowbreak{}Message\allowbreak{}Content} und übergeben
diesen an den \texttt{TwilioMessageSender}. Die Abhängigkeit
vom Twilio Java SDK bleibt dadurch auf den ausgehenden
Kommunikationsadapter beschränkt.

Das Content Template enthält folgende Nachricht:

\begin{quote}
\small\ttfamily
Guten Tag \{\{1\}\},\par\medskip
für Ihren Auftrag \{\{2\}\} ist die Lieferung von
\{\{3\}\} (\{\{4\}\}) am \{\{5\}\} im Zeitfenster
\{\{6\}\} geplant.\par\medskip
Lieferadresse:\par
\{\{7\}\}\par\medskip
Bitte bestätigen Sie Ihren Liefertermin über folgenden Link:\par
\{\{8\}\}\par\medskip
Vielen Dank.
\end{quote}

Die Platzhalter werden beim Versand mit den Daten des jeweiligen
Auftrags und der Bestätigungsanfrage befüllt. Ihre Zuordnung ist
in Tabelle~\ref{tab:twilio-template-variablen} dargestellt.

\begin{table}[htbp]
    \centering
    \caption{Variablen des Twilio Content Templates}
    \label{tab:twilio-template-variablen}
    \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}X@{}}
        \toprule
        Platzhalter & Inhalt \\
        \midrule
        \texttt{\{\{1\}\}} & Kundenname \\
        \texttt{\{\{2\}\}} & externe Auftragsnummer \\
        \texttt{\{\{3\}\}} & Produkt \\
        \texttt{\{\{4\}\}} & Bestellmenge \\
        \texttt{\{\{5\}\}} & Lieferdatum \\
        \texttt{\{\{6\}\}} & Lieferzeitfenster \\
        \texttt{\{\{7\}\}} & Lieferadresse \\
        \texttt{\{\{8\}\}} & individueller Bestätigungslink \\
        \bottomrule
    \end{tabularx}
\end{table}

Für den Versand übermittelt der \texttt{TwilioMessageSender}
die konfigurierte Content-Template-SID als \texttt{ContentSid}
an die Twilio Messages API. Die auftragsspezifischen Inhalte
werden den Platzhaltern des Templates zugeordnet, als
\texttt{ContentVariables} serialisiert und zusammen mit den
Absender- und Empfängerdaten übertragen. Anhand der SID lädt
Twilio das hinterlegte Template und setzt die übergebenen Werte
in die entsprechenden Platzhalter ein.

Abbildung~\ref{fig:twilio-nachrichtenbeispiele} zeigt die aus
dem gemeinsamen Content Template erzeugten Nachrichten in den
beiden Kommunikationskanälen.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.44\textwidth}
        ...
        \includegraphics[
            ...
            keepaspectratio
        ]{figures/twilio-sms-beispiel.jpg}
        \caption{Versand als SMS}
        \label{fig:twilio-sms-beispiel}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.44\textwidth}
        \centering
        \includegraphics[
            height=0.34\textheight,
            keepaspectratio
        ]{figures/twilio-whatsapp-beispiel.jpg}
        \caption{Versand über WhatsApp}
        \label{fig:twilio-whatsapp-beispiel}
    \end{subfigure}

    \caption{Über Twilio versendete Bestätigungsanfragen}
    \label{fig:twilio-nachrichtenbeispiele}

    \par\smallskip
    {\footnotesize\textit{Quelle: Eigene Darstellung.}}
\end{figure}

Die kanalspezifische Verarbeitung unterscheidet sich vor allem
bei der Adressierung. Beide Kanäle verwenden die im Auftrag
gespeicherte Kundenrufnummer als Empfänger. Für SMS werden
Absender und Empfänger im internationalen E.164-Format
übertragen. Bei WhatsApp ergänzt der
\texttt{TwilioMessageSender} beide Adressen um das von Twilio
erwartete Präfix \texttt{whatsapp:}. Ein bereits vorhandenes Präfix
wird dabei nicht doppelt gesetzt. Für SMS und WhatsApp können
getrennte Absenderkennungen konfiguriert werden.

Die Kommunikation mit der Twilio API erfolgt über einen
\texttt{TwilioRestClient}. Die dafür benötigte Account SID,
das Authentifizierungstoken, die Absender für SMS und WhatsApp
sowie die Content-Template-SID werden über Umgebungsvariablen
bereitgestellt. Diese Konfiguration gilt im aktuellen
Projektstand anwendungsweit.

Vor jedem Versand prüft der Adapter, ob die für den gewählten
Kanal erforderliche Konfiguration vollständig ist. Zusätzlich
werden Absender und Empfänger auf ein gültiges E.164-Format
geprüft. Fehlt die Kundenrufnummer oder besitzt eine der
verwendeten Adressen ein ungültiges Format, bricht die
Anwendung den Vorgang vor dem Aufruf der Twilio API ab.

Fehler bei der Serialisierung der Template-Variablen und
fehlgeschlagene Aufrufe der Twilio API werden als gemeinsame
\texttt{Notification\allowbreak{}Delivery\allowbreak{}Exception} mit Angabe des
betroffenen Kommunikationskanals an die Anwendungslogik
weitergegeben. Um personenbezogene Daten in den Log-Ausgaben zu schützen, werden
Telefonnummern maskiert und nur ihre letzten vier Ziffern
angezeigt.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    Kundenzugriff
      & Zugriff auf eine konkrete Bestätigungsanfrage begrenzen
...
Bestätigungsanfragen und Kundenreaktionen. Mandantenspezifische technische
...
eine konkrete Bestätigungsanfrage, der in
...
den Dashboard-Zugriff wird somit die bereits bestimmte Mandantenidentität
übertragen, nicht das langfristig gültige Authentifizierungsgeheimnis.

Fordert DISPO einen Dashboard-Zugriff an, erzeugt der Avisierungsservice einen
...
weiterer Dashboard-Zugriffe verwendet der Browser ausschließlich die vom
...
zustandsändernde Dashboard-Zugriffe zusätzlich durch CSRF-Schutz abgesichert.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \draw[message] (0,-1.4) --
      node[midway,above,msglabel] {Dashboard-Zugriff\\+ \texttt{X-API-Key}}
      (6,-1.4);
...
Werden für einen Auftrag nacheinander mehrere Bestätigungsanfragen erzeugt,
...
Anfrage den höchsten Datenbankidentifikator unter den Bestätigungsanfragen
...
ältere Bestätigungslinks ihre Gültigkeit, ohne dass die zugehörigen historischen
...
Bestätigungsanfrage angelegt wurde. Dies ist bewusst vorgesehen, damit der
...
Fahrzeugposition wird zusätzlich nur dann bereitgestellt, wenn entsprechende
...
jeweils neueste Bestätigungsanfrage erschweren das Erraten gültiger Tokens
...
Die in den vorhergehenden Abschnitten erzeugten Auftrags- und
Avisierungsdaten müssen der Disposition für die operative Bearbeitung
...
Anfrage mit der höchsten internen ID. Frühere Bestätigungsanfragen bleiben
...
Versandstatus einer einzelnen Bestätigungsanfrage. Wird kein Startdatum
...
Bestätigungsanfrage, Gruppierung, Sortierung und Paginierung kombiniert
...
Bestätigungsanfrage vor, bleibt dieser Bestandteil leer. Eine separate
...
einer einzelnen Bestätigungsanfrage zu unterscheiden. Für diese werden die
...
\begin{table}[htbp]
  ...
  \caption{Darstellungsstatus einzelner Bestätigungsanfragen}
  \label{tab:anfrage-darstellungsstatus}
  \begin{tabularx}{\textwidth}{@{}
    >{\raggedright\arraybackslash}X
    >{\raggedright\arraybackslash}p{0.19\textwidth}@{}}
    \toprule
    \textbf{Zustand der Bestätigungsanfrage}
...
\subsection{Lieferverfolgung und Geocoding (OKB)}
\label{sec:lieferverfolgung}

Die Lieferverfolgung führt Positionsdaten aus zwei
unterschiedlichen Quellen zusammen. Die aktuelle Fahrzeugposition
wird bei Bedarf über eine DISPO-Schnittstelle abgerufen. Die
Koordinaten des Lieferziels werden dagegen aus der im Auftrag
gespeicherten Lieferadresse ermittelt. Der Avisierungsservice
ordnet beide Informationen über die zugehörige
Bestätigungsanfrage dem jeweiligen Auftrag zu und stellt sie über
getrennte tokenbasierte Kundenschnittstellen bereit.

Tracking-Daten stehen ausschließlich am hinterlegten Liefertag
zur Verfügung. Zur Prüfung vergleicht die Anwendung das Datum des
\texttt{DeliverySlot} mit dem aktuellen Datum. Dieses wird über
einen eingebundenen \texttt{Clock} ermittelt. Dadurch bleibt die
zeitabhängige Logik unabhängig von direkten Systemzeitaufrufen und
kann in automatisierten Tests mit einer kontrollierten Zeitquelle
überprüft werden.

\subsubsection{Daten zur Lieferverfolgung und Fahrzeugposition}

Die Fahrzeugposition wird nicht im Avisierungsservice gespeichert,
sondern bei Bedarf aus DISPO abgerufen. Ausgangspunkt
der Abfrage ist der übergebene Bestätigungstoken. Der
\texttt{GetDriverLocationService} ordnet diesen der aktuellen
Bestätigungsanfrage und dem zugehörigen Auftrag zu. Unbekannte
Token sowie Token inzwischen abgelöster Anfragen können daher
nicht für den Zugriff auf Tracking-Daten verwendet werden.

Die Abfrage der Fahrzeugposition erfolgt ausschließlich am
Liefertag. Hierzu wird das im \texttt{DeliverySlot} gespeicherte
Lieferdatum mit dem aktuellen Datum verglichen. Stimmen beide
überein, übergibt der \texttt{GetDriverLocationService} die
externe Auftragsnummer an den \texttt{LocationTrackingService}.
Sie dient als Zuordnungsschlüssel, da sie den Auftrag auch in DISPO eindeutig identifiziert. Vor oder nach
dem Liefertag erfolgt keine Anfrage an die externe
Tracking-Schnittstelle.

Der \texttt{LocationTrackingService} kapselt den Zugriff auf
die DISPO-Schnittstelle und stellt die Fahrzeugposition als
optionales Ergebnis bereit. Verfügbare Positionsdaten werden in
ein \texttt{GeoCoordinate} überführt. Abweichende
Antwortsituationen und Kommunikationsfehler werden innerhalb des
Adapters behandelt und protokolliert, sodass der aufrufende
Application Service über eine einheitliche Schnittstelle auf
das Ergebnis zugreifen kann.

Durch die bedarfsgesteuerte Abfrage erhält der
Avisierungsservice die zum jeweiligen Anfragezeitpunkt von DISPO
gelieferte Fahrzeugposition. Die aktuellen Positionsdaten können
anschließend über die tokenbasierte Kundenschnittstelle
ausgegeben werden.

\subsubsection{Zielkoordinaten und Geocoding}

Die Lieferadresse liegt im Auftrag zunächst als Text vor. Zur
Ermittlung der geografischen Zielposition löst der
\texttt{Delivery\allowbreak{}Address\allowbreak{}Coordinate\allowbreak{}Resolver} diese Adresse in
Längen- und Breitengrad auf. Der Zugriff auf den externen
Geocoding-Dienst erfolgt über einen ausgehenden Port. Dessen
aktuelle Implementierung verwendet die Nominatim Search API von
OpenStreetMap.

Für die prototypische Umsetzung wurde Nominatim ausgewählt, da
der Dienst eine dokumentierte HTTP-basierte Suchschnittstelle
für OpenStreetMap-Daten bereitstellt und ohne ein zusätzliches
providerspezifisches SDK angebunden werden kann. Dadurch ließ
sich die Adressauflösung über den bereits verwendeten
HTTP-Client mit geringem zusätzlichem Integrationsaufwand
umsetzen. 

Vor dem externen Aufruf wird die Lieferadresse durch den
\texttt{AddressNormalizer} aufbereitet. Dabei werden unter
anderem Umlaute und das Zeichen \texttt{ß} vereinheitlicht, die
Straßenabkürzung \texttt{Str.} erweitert und überflüssige
Leerzeichen entfernt. Zusätzlich wird eine stärker
normalisierte Darstellung als Cache-Schlüssel erzeugt. Dadurch
können unterschiedliche Schreibweisen derselben Adresse
möglichst demselben Cache-Eintrag zugeordnet werden.

Der \texttt{GeocodingClient} übermittelt die aufbereitete
Adresse als freie Suchanfrage an Nominatim. Die Antwort wird im
Format \texttt{jsonv2} angefordert und mithilfe eines
Länderfilters auf Ergebnisse aus Deutschland beschränkt. Aus
der Antwort wird der erste Treffer verwendet. Dessen Werte für
Längen- und Breitengrad werden in ein
\texttt{GeoCoordinate} überführt. Basis-URL, Suchpfad,
Länderfilter, bevorzugte Sprache, maximale Ergebnisanzahl und
\texttt{User-Agent} können über die Anwendungskonfiguration
festgelegt werden
\parencite{nominatimSearchApi}.

Ermittelte Koordinaten werden standardmäßig für 60 Minuten in
einem lokalen In-Memory-Cache gespeichert. Eine erneute
Auflösung derselben Adresse erfordert dadurch während dieses
Zeitraums keinen weiteren Zugriff auf den externen
Geocoding-Dienst. Der Cache gehört zur laufenden
Anwendungsinstanz und wird bei einem Neustart neu aufgebaut.

Kann Nominatim für eine Lieferadresse keine Zielkoordinate
ermitteln, erzeugt der
\texttt{Delivery\allowbreak{}Address\allowbreak{}Coordinate\allowbreak{}Resolver} im aktuellen
Projektstand eine deterministische Ersatzkoordinate im Raum
Würzburg. Dadurch bleibt die Lieferverfolgung während der
Entwicklung und Erprobung auch dann reproduzierbar, wenn der
Geocoding-Dienst nicht erreichbar ist oder kein verwendbares
Ergebnis liefert. Da die Ersatzkoordinate keine verifizierte
geografische Position der Lieferadresse darstellt, wäre dieser
Mechanismus für einen produktiven Einsatz entsprechend
anzupassen.

\subsubsection{Bereitstellung für die Bestätigungsseite}

Der Avisierungsservice stellt die Tracking-Daten über zwei
tokenbasierte Kundenschnittstellen bereit. Beide Zugriffe
verwenden den Bestätigungstoken zur Ermittlung der aktuellen
Bestätigungsanfrage und des zugehörigen Auftrags.

Die GET-Schnittstelle

\begin{center}
\texttt{/api/customer/confirmations/\{token\}/tracking-info}
\end{center}

liefert das Kennzeichen \texttt{trackingAvailable} und optional
die Koordinaten des Lieferziels. Dabei repräsentiert
\texttt{targetLocationX} den Längengrad und
\texttt{targetLocationY} den Breitengrad. Stimmt das aktuelle
Datum nicht mit dem Liefertag überein, ist
\texttt{trackingAvailable} auf \texttt{false} gesetzt und beide
Koordinatenwerte sind \texttt{null}. Eine Geocoding-Anfrage wird
in diesem Fall nicht ausgeführt.

Die aktuelle Fahrzeugposition wird über eine getrennte
GET-Schnittstelle bereitgestellt:

\begin{center}
\texttt{/api/customer/confirmations/\{token\}/driver-location}
\end{center}

Ist am Liefertag eine Position verfügbar, liefert
die Schnittstelle den Längengrad als \texttt{locationX} und den
Breitengrad als \texttt{locationY}. Liefert der
\texttt{GetDriverLocationService} kein Ergebnis, antwortet die
Schnittstelle ohne Positionsdaten mit dem HTTP-Statuscode
\texttt{404 Not Found}. Dies gilt insbesondere außerhalb des
Liefertags, bei einer nicht verfügbaren Fahrzeugposition und bei
einem fehlgeschlagenen DISPO-Aufruf.

Die Antwort der Tracking-Info-Schnittstelle sowie erfolgreiche
Antworten der Fahrzeugpositionsschnittstelle werden mit
\texttt{Cache-Control: no-store} versehen. Dadurch weist das
Backend aufrufende Clients und zwischengeschaltete Systeme an,
die bereitgestellten Tracking-Daten nicht zwischenzuspeichern.

Die Trennung der beiden Schnittstellen berücksichtigt die
unterschiedliche Veränderungshäufigkeit der Daten. Die
Zielkoordinaten bleiben für eine Lieferadresse weitgehend
stabil, während die Fahrzeugposition bei jedem Abruf erneut von
DISPO angefordert wird. Dadurch lässt sich die aktuelle
Fahrzeugposition wiederholt abfragen, ohne zugleich eine erneute
Geocodierung der Lieferadresse auszulösen.
