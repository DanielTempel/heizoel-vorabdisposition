...
Für die mobilen Kommunikationskanäle SMS und WhatsApp wurde
Twilio als externer Messaging-Provider ausgewählt. Twilio
ermöglicht die Anbindung beider Kanäle über eine gemeinsame
Messaging-Schnittstelle und unterstützt zentral verwaltete
Content Templates. Dadurch kann die bestehende
Kommunikationsarchitektur um beide Kanäle erweitert werden,
ohne zwei voneinander unabhängige Provider-Integrationen
implementieren zu müssen
\parencite{twilioContentTemplates,twilioMessagesResource}.

Nach der in Abschnitt~\ref{sec:kommunikationsabstraktion}
beschriebenen Kanalauswahl übernimmt der
\texttt{SmsNotificationSender} beziehungsweise der
\texttt{WhatsAppNotificationSender} die kanalspezifische
Weiterleitung. Beide Komponenten erzeugen aus dem Auftrag und
der Bestätigungsanfrage einen gemeinsamen
\texttt{ConfirmationMessageContent} und übergeben diesen an den
\texttt{TwilioMessageSender}. Die Abhängigkeit vom Twilio Java
SDK bleibt dadurch auf den ausgehenden Kommunikationsadapter
beschränkt.

Für beide Kanäle wird das im Twilio Content Template Builder
angelegte Template \texttt{disposition\_test} verwendet. Die
Anwendung referenziert dieses Template beim Versand nicht über
seinen Namen, sondern über die konfigurierte
Content-Template-SID. Das Template besitzt folgenden Aufbau:
...
Auftrags und der Bestätigungsanfrage befüllt:

\begin{itemize}
    \item \texttt{\{\{1\}\}}: Kundenname,
    \item \texttt{\{\{2\}\}}: externe Auftragsnummer,
    \item \texttt{\{\{3\}\}}: Produkt,
    \item \texttt{\{\{4\}\}}: Bestellmenge,
    \item \texttt{\{\{5\}\}}: Lieferdatum,
    \item \texttt{\{\{6\}\}}: Lieferzeitfenster,
    \item \texttt{\{\{7\}\}}: Lieferadresse,
    \item \texttt{\{\{8\}\}}: individueller Bestätigungslink.
\end{itemize}

Der \texttt{TwilioMessageSender} übergibt die konfigurierte
Content-Template-SID als \texttt{ContentSid} und die
auftragsspezifischen Werte als serialisierte
\texttt{ContentVariables} an die Twilio Messages API. Twilio
ermittelt anhand der SID das hinterlegte Template und ersetzt
die Platzhalter durch die übergebenen Werte. SMS und WhatsApp
verwenden dadurch dieselben fachlichen Informationen.

Die kanalspezifischen Unterschiede betreffen insbesondere die
Adressierung. SMS-Absender und -Empfänger werden im
internationalen E.164-Format übertragen. Für WhatsApp ergänzt
der Adapter das Präfix \texttt{whatsapp:}. Ein bereits
vorhandenes Präfix wird nicht erneut hinzugefügt. Für beide
Kanäle können getrennte Absender konfiguriert werden.

Die Verbindung zu Twilio wird über einen
\texttt{TwilioRestClient} aufgebaut. Account SID,
Authentifizierungstoken, SMS-Absender, WhatsApp-Absender und
Content-Template-SID werden über Umgebungsvariablen
bereitgestellt. Im Gegensatz zur mandantenspezifischen
SMTP-Konfiguration gilt die Twilio-Konfiguration im aktuellen
Projektstand anwendungsweit.

Vor dem Versand prüft der Adapter, ob die erforderliche
Provider-Konfiguration vollständig ist und ob Absender und
Empfänger ein gültiges Format besitzen. Fehler bei der
Serialisierung der Template-Variablen oder beim Aufruf der
Twilio API werden als kanalbezogene
\texttt{NotificationDeliveryException} an die Anwendungslogik
weitergegeben. Telefonnummern werden in den Log-Ausgaben
maskiert, sodass nur die letzten Ziffern sichtbar bleiben.

Für die Entwicklung kann die Twilio WhatsApp Sandbox verwendet
werden. Diese ist ausschließlich für Testzwecke vorgesehen und
erlaubt den Versand nur an Empfänger, die zuvor mit der Sandbox
verbunden wurden \parencite{twilioWhatsAppSandbox}. Für einen
produktiven Einsatz sind entsprechend konfigurierte Absender
und ein für WhatsApp freigegebenes Content Template
erforderlich.

Die Twilio-Integration umfasst im aktuellen Projektstand die
initiale Bestätigungsanfrage über SMS und WhatsApp. Eine
Eingangsbestätigung nach der Kundenreaktion wird über diese
Kanäle nicht versendet. Zudem wird kein Twilio-Status-Callback
verarbeitet. Ein erfolgreicher API-Aufruf bestätigt daher die
Annahme der Nachricht durch Twilio, jedoch nicht ihre
endgültige Zustellung an das Endgerät.
...
Die Lieferverfolgung verbindet Positionsdaten aus zwei
unterschiedlichen Quellen. Die aktuelle Fahrerposition wird über
eine DISPO-Schnittstelle abgefragt, während die Zielkoordinaten
aus der im Auftrag gespeicherten Lieferadresse ermittelt werden.
Der Avisierungsservice führt beide Informationen über die
jeweilige Bestätigungsanfrage zusammen und stellt sie der
Bestätigungsseite über getrennte Kundenschnittstellen bereit.

Tracking-Informationen werden ausschließlich am hinterlegten
Liefertag als verfügbar ausgewiesen. Die zeitliche Prüfung
verwendet den in die Anwendung eingebundenen \texttt{Clock} und
vergleicht das aktuelle Datum mit dem Datum des
\texttt{DeliverySlot}. Dadurch kann die Datumsabhängigkeit in
automatisierten Tests mit einer kontrollierten Zeitquelle
überprüft werden.

\subsubsection{Daten zur Lieferverfolgung und Fahrerposition}

Die aktuelle Fahrerposition wird nicht im Avisierungsservice
gespeichert. Stattdessen fragt der
\texttt{LocationTrackingService} sie bei Bedarf über die
konfigurierte DISPO-Tracking-Schnittstelle ab. Zur Zuordnung wird
die externe Auftragsnummer verwendet, da diese den Auftrag auch
im führenden DISPO-System identifiziert.

Vor der Abfrage löst der \texttt{GetDriverLocationService} den
übergebenen Bestätigungstoken auf. Berücksichtigt wird nur die
aktuelle Bestätigungsanfrage des Auftrags. Ist der Token
unbekannt oder gehört er zu einer inzwischen abgelösten Anfrage,
wird der Zugriff abgelehnt.

Die DISPO-Abfrage erfolgt nur, wenn das im
\texttt{DeliverySlot} gespeicherte Datum dem aktuellen Datum
entspricht. Vor oder nach dem Liefertag wird keine
Fahrerposition angefordert. Liefert DISPO gültige Positionsdaten,
werden diese als Längen- und Breitengrad übernommen. Ist keine
Position verfügbar oder schlägt der Aufruf der externen
Schnittstelle fehl, wird keine Fahrerposition zurückgegeben.
Ein technischer Fehler der DISPO-Abfrage führt damit nicht zum
Abbruch des gesamten Kundenvorgangs.

Durch die bedarfsgesteuerte Abfrage erhält die Bestätigungsseite
den jeweils aktuellen, von DISPO bereitgestellten Standort. Da
die Position nicht lokal historisiert wird, bildet der
Avisierungsservice selbst jedoch keinen zurückgelegten
Fahrweg ab.

\subsubsection{Zielkoordinaten und Geocoding}

Die Lieferadresse liegt im Auftrag zunächst als Text vor. Für
die Darstellung auf einer Karte wird sie durch den
\texttt{DeliveryAddressCoordinateResolver} in geografische
Koordinaten aufgelöst. Die eigentliche Geocoding-Anfrage erfolgt
über einen ausgehenden Port, dessen aktuelle Implementierung den
Nominatim-Dienst von OpenStreetMap verwendet.

Vor der Anfrage wird die Lieferadresse normalisiert. Dabei
werden unter anderem Umlaute, das Zeichen \texttt{ß},
abweichende Schreibweisen und überflüssige Leerzeichen
vereinheitlicht. Aus der normalisierten Adresse wird außerdem
ein stabiler Cache-Schlüssel erzeugt. Dadurch führen
unterschiedliche Schreibweisen derselben Adresse möglichst zur
gleichen Cache-Zuordnung.

Der \texttt{GeocodingClient} übermittelt die Adresse als freie
Suchanfrage an die Nominatim Search API. Die Antwort wird im
Format \texttt{jsonv2} angefordert und auf deutsche Ergebnisse
beschränkt. Verwendet wird der erste zurückgegebene Treffer,
dessen Werte für Längen- und Breitengrad in ein
\texttt{GeoCoordinate} überführt werden. Basis-URL,
Suchpfad, Sprache, Länderfilter, Ergebnisanzahl und
\texttt{User-Agent} sind über die Anwendungskonfiguration
einstellbar
\parencite{nominatimSearchApi}.

Ermittelte Koordinaten werden standardmäßig für 60 Minuten in
einem lokalen In-Memory-Cache gespeichert. Wiederholte Anfragen
für dieselbe Adresse müssen dadurch nicht erneut an den externen
Dienst gesendet werden. Der Cache gilt nur innerhalb der
laufenden Anwendungsinstanz und wird bei einem Neustart nicht
beibehalten.

Liefert der Geocoding-Dienst keinen verwendbaren Treffer oder
ist er technisch nicht erreichbar, erzeugt der Resolver für die
prototypische Darstellung eine deterministische
Ersatzkoordinate im Raum Würzburg. Dieselbe normalisierte
Adresse führt dabei stets zur gleichen Ersatzposition. Diese
Position ermöglicht weiterhin eine Kartendarstellung, stellt
jedoch keine verifizierte geografische Lage der Lieferadresse
dar. Eine leere oder nicht normalisierbare Adresse führt dagegen
zu keinem Zielpunkt.

Die öffentliche Nominatim-Instanz unterliegt
Nutzungsbeschränkungen. Die Anwendung setzt einen identifizierbaren
\texttt{User-Agent} und reduziert wiederholte Zugriffe durch
Caching. Für einen produktiven Einsatz mit höherem
Anfragevolumen wären zusätzlich die geltenden Zugriffslimits zu
berücksichtigen und gegebenenfalls eine eigene Instanz oder ein
geeigneter kommerzieller Geocoding-Dienst einzusetzen
\parencite{nominatimUsagePolicy}.

\subsubsection{Bereitstellung für die Bestätigungsseite}

Die Tracking-Daten werden über zwei tokenbasierte
Kundenschnittstellen bereitgestellt. Die Schnittstelle

\begin{center}
\texttt{/api/customer/confirmations/\{token\}/tracking-info}
\end{center}

liefert das Kennzeichen \texttt{trackingAvailable} sowie
optional die Koordinaten des Lieferziels. Dabei entspricht
\texttt{targetLocationX} dem Längengrad und
\texttt{targetLocationY} dem Breitengrad. Außerhalb des
Liefertags ist \texttt{trackingAvailable} auf
\texttt{false} gesetzt und beide Koordinatenwerte sind
\texttt{null}.

Die aktuelle Fahrerposition wird getrennt über

\begin{center}
\texttt{/api/customer/confirmations/\{token\}/driver-location}
\end{center}

abgerufen. Ist am Liefertag eine Position verfügbar, liefert die
Schnittstelle den Längen- und Breitengrad als
\texttt{locationX} und \texttt{locationY}. Liegt keine Position
vor oder wird die Schnittstelle außerhalb des Liefertags
aufgerufen, antwortet sie ohne Positionsdaten mit dem
HTTP-Status \texttt{404 Not Found}.

Beide Antworten werden mit \texttt{Cache-Control: no-store}
bereitgestellt, damit Browser und zwischengeschaltete Systeme
keine möglicherweise veralteten Positionsdaten speichern. Die
Bestätigungsseite lädt die Tracking-Informationen nach einer
erfolgreichen Terminbestätigung. Nur wenn Tracking verfügbar ist
und Zielkoordinaten vorliegen, wird zusätzlich die
Fahrerposition angefordert.

Die getrennten Schnittstellen erlauben es, die weitgehend
stabile Zielposition unabhängig von der veränderlichen
Fahrerposition zu behandeln. Über den Button
\glqq Aktualisieren\grqq{} kann die Bestätigungsseite die
Fahrerposition erneut abrufen, ohne die Zieladresse erneut
geocodieren zu müssen.
