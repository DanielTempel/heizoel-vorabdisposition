...
\section{Webfrontend -- Konzeption und Implementierung}
...
der Kundenlink unmittelbar zum betreffenden Vorgang. Die in Kapitel 4 beschriebene
...
Lieferadresse, Lieferzeitfenster, Antwortfrist und Rückbestätigungsstatus. Über den jeweiligen
...
\begin{figure}[htbp]
  ...
  \par\smallskip
  {\footnotesize Quelle: Eigene Aufnahme der Anwendung mit Demonstrationsdaten.\par}
\end{figure}
...
auf die Fortsetzung eines bestehenden Avisierungsvorgangs begrenzt. Insbesondere ist eine
...
Sie wird im Webfrontend nur im Zusammenhang mit einem bestätigten Liefertermin angefordert. Die
Seite fragt zunächst ab, ob Tracking verfügbar ist und ob Zielkoordinaten vorliegen. Auf dieser
Grundlage wird die Kartenansicht eingeblendet; die Fahrerposition wird über einen weiteren
Aufruf geladen. Bei einer abgelehnten oder ohne Rückmeldung abgeschlossenen Anfrage wird keine
Karte angezeigt.
...
auf die Bereitstellung am Liefertag. Die Herkunft der Zielkoordinaten und die Anbindung der
Fahrerposition werden in Abschnitt~\ref{sec:lieferverfolgung} behandelt. Das Webfrontend
verwendet die bereitgestellten Koordinaten für die Darstellung und führt selbst keine
Adressauflösung durch.

\subsubsection{Karten- und Positionsdarstellung}

Die Kartenansicht verwendet Leaflet mit React Leaflet und Kartenkacheln von OpenStreetMap.
Unterschiedliche Symbole kennzeichnen das Fahrzeug und das Lieferziel. Der Kartenausschnitt
wird an die vorhandenen Positionen angepasst. Liegt zunächst nur das Ziel vor, kann dieses
bereits angezeigt werden; bei verfügbarer Fahrerposition umfasst die Ansicht beide Punkte. Die
zugehörige Lieferadresse bleibt als Text sichtbar, damit das Kartenziel dem Auftrag zugeordnet
werden kann.

Zwischen Fahrzeug und Ziel wird eine Verbindungslinie eingezeichnet. Zusätzlich berechnet das
Webfrontend aus den Koordinaten die Entfernung entlang der Erdoberfläche als
Luftlinienentfernung und zeigt sie gerundet in Kilometern an. Die Darstellung bietet damit eine
räumliche Orientierung. Sie enthält weder eine berechnete Straßenroute noch eine Prognose der
Ankunftszeit.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{figures/frontend-tracking.png}
  ...
  \par\smallskip
  {\footnotesize Quelle: Eigene Aufnahme der Anwendung mit Demonstrationsdaten. Kartengrundlage: OpenStreetMap-Mitwirkende.\par}
\end{figure}

\subsubsection{Aktualisierung und Fehlerbehandlung}

Die erste Fahrerposition wird nach dem Laden der verfügbaren Tracking-Informationen automatisch
angefordert. Nach einem erfolgreichen Abruf kann der Kunde über \enquote{Aktualisieren} eine
weitere Position laden. Währenddessen ist die Schaltfläche deaktiviert und zeigt den laufenden
Abruf an. Neue Koordinaten aktualisieren die Fahrzeugmarkierung und die daraus abgeleitete
...
Scheitert die Aktualisierung einer bereits geladenen Fahrerposition, behält die Seite den
