...
class SmsNotificationSenderTest {
    ...
    @Test
    void sendConfirmationRequestBuildsSharedTemplateVariables() {
        ...
        SmsNotificationSender sender = new SmsNotificationSender(properties, twilioMessageSender);

        Order order = order();
        ConfirmationRequest confirmationRequest = confirmationRequest(order, CommunicationChannel.SMS);

        sender.sendConfirmationRequest(order, confirmationRequest);

        verify(twilioMessageSender).sendSms(
                "A-1024",
                "+491701234567",
